...
void timerInit(void)
{
    ...
    T4CONbits.TSIDL = 1; // Discontinue timer operation when device enters Idle mode
    T4CONbits.TGATE = 0; // Gated time accumulation disabled
    T4CONbits.TCKPS = 0b00; // 1:1 prescale value
    T4CONbits.TCS = 0; // Internal clock as source
    TMR4 = 0x00; // clear Timer4
    PR4 = 0xFFFF; // load an arbitrary value for comparison (not 0)
